...
CPin::CPin(char *message, char *title, bool repeat)
{
...
LRESULT CPin::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	edit2.SendMessage(EM_SETLIMITTEXT, 8, 0);
	if (repeat)
		edit2.SetFocus();
	else
...
LRESULT CPin::OnClickedOK(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	...
	if (strnlen_s(PIN, 99) != 8) {
		ShowToolTip(edit, L"Il PIN deve essere di 8 cifre", L"Lunghezza PIN errata");
		return TRUE;
	}
	if (repeat) {
		char PIN2[100];
		...
		if (strnlen_s(PIN2, 99) != 8) {
			ShowToolTip(edit2, L"Il PIN deve essere di 8 cifre", L"Lunghezza PIN errata");
			return TRUE;
		}
